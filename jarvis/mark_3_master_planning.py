...
def get_mark_3_macro_roadmap() -> Dict[str, Any]:
    definitions = [
        ...
        (136, "Mark 3 Governed Research Execution Control Plane", "Normalize research requests, re-check policy and capability, and prepare execution candidates without faking adapters.", ["research normalization", "policy and approval decision", "capability contract", "outcome/failure hooks"], ["missing web/GitHub capability returns setup_required rather than fake research"], False),
        (137, "Mark 3 Local Docs/Repo Research Adapter", "Connect bounded local read-only research for docs/local_repo through one exact safe file scope.", ["local docs/repo read adapter", "exact scope validation", "secret/path/symlink guards", "no web or GitHub"], ["only one exact allowed local file can be read and research /candidate never rehydrates previews by id"], False),
        (138, "Mark 3 Product/Revenue Factory", "Build a governed opportunity, product, monetization, experiment, measurement, and kill/continue control-plane.", ["opportunity candidate", "niche validation", "product blueprint", "landing and pricing candidates", "unit economics", "measurement plan", "kill or continue gate"], ["no fake revenue or costs; publication, deploy, payments, domains, identity, and external providers remain Level 4 gated and unexecuted"], False),
        (139, "Mark 3 Local Routine Scheduler + Personal/Family Ops", "Prepare governed local routines, personal ops, family ops, authorized account assistance, and health candidates without real scheduler or account access.", ["routine candidates", "daily and weekly plan previews", "personal/family ops candidates", "authorized account assistance", "password manager and 2FA checklists"], ["no real scheduler, cron, worker, email, Calendar, Gmail, Contacts, account access, password storage, 2FA bypass, cookie/token/session use, or fake completion"], False),
        (140, "Mark 3 Moonshot Lab + Research/Experiment Engine", "Create an honest, stage-gated engine for ambitious research, experiments, and prototypes.", ["moonshot intake", "hypothesis framing", "research experiment plans", "prototype candidates", "evidence scoring", "uncertainty labels", "stage gates"], ["no experiment execution, network, providers, installs, publication, deploy, money, fake breakthrough, benchmark, result, or capability"], False),
        (141, "Mark 3 Release Candidate + Pilot", "Close Mark 3 as a controlled release candidate and prepare the first real local pilot without executing it yet.", ["capability matrix", "readiness matrix", "dangerous-route audit", "approval-path audit", "prepare-only gated smoke", "pilot plan", "operational runbook"], ["Mark 3 is ready as a controlled release candidate, not free autonomy; pilot is local, useful, approved, and non-production"], False),
    ]
